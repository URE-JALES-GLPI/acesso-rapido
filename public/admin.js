...
function renderTileList() {
  ...
  tilesCache.forEach((tile, index) => {
    ...
    row.addEventListener('dragover', (e) => {
      ...
      const targetIdx = arr.findIndex(t => t.id === tile.id);
      arr.splice(before ? targetIdx : targetIdx + 1, 0, tilesCache[dragIndex]);
      tilesCache = arr;
...
async function persistTileOrder() {
  await fetch('/api/tiles/reorder', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ order: tilesCache.map(t => t.id) })
  });
}
